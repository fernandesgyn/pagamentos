...
final class ObrigacaoRegra{
    public static function validarNumero(int $tipoId,string $numero):void{
        $st=Database::connection()->prepare("SELECT nome FROM tipos_obrigacao WHERE id=? LIMIT 1");
        $st->execute([$tipoId]);$tipo=(string)$st->fetchColumn();
        if($tipo==='')throw new InvalidArgumentException('Tipo de obrigação inválido.');
