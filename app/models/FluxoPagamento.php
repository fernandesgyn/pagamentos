...
final class FluxoPagamento
{
    ...
    {
        return $this->db->query("SELECT * FROM tipos_recurso WHERE ativo=1 ORDER BY codigo,nome")->fetchAll();
    }
    ...
    public function criarObrigacao(array $d): int
    {
        ...
        if ($valor === null || $valor <= 0) {
            throw new InvalidArgumentException('Informe o Valor Total da Obrigação.');
        }
        if (!$fontes) {
            throw new InvalidArgumentException('Informe ao menos uma Fonte de recurso.');
        }
        if (!$naturezas) {
            throw new InvalidArgumentException('Informe ao menos uma Natureza da despesa.');
        }
        ...
        try {
            ...
            if ((string)$e->getCode() === '23000') {
                throw new RuntimeException('Já existe obrigação com este tipo, número e ano ou algum vínculo informado é inválido.');
            }
            throw $e;
        } catch (Throwable $e) {
            if ($this->db->inTransaction()) $this->db->rollBack();
    ...
    public function criarDocumento(array $d): int
    {
        ...
        if ($liquido > $bruto) {
            throw new InvalidArgumentException('O valor líquido não pode ser maior que o valor bruto.');
        }

        $q = $this->db->prepare("SELECT 1 FROM obrigacoes WHERE id=? AND fornecedor_id=? AND ativo=1");
        $q->execute([$obrigacao, $fornecedor]);
        if (!$q->fetchColumn()) {
            throw new RuntimeException('A obrigação selecionada não pertence ao fornecedor informado.');
        }
        ...
        try {
            ...
            if ($status <= 0) {
                throw new RuntimeException('Status inicial de inspeção não configurado.');
            }
            ...
            if ($this->db->inTransaction()) $this->db->rollBack();
            if ((string)$e->getCode() === '23000') {
                throw new RuntimeException('Já existe documento com este número para a obrigação/tipo informado ou algum vínculo é inválido.');
            }
            throw $e;
        } catch (Throwable $e) {
            if ($this->db->inTransaction()) $this->db->rollBack();
    ...
    {
        $sql = "SELECT d.*,td.nome tipo_documento,o.numero obrigacao_numero,o.ano obrigacao_ano,
    ...
    public function obrigacoesFornecedor(int $fornecedorId): array
    {
        $st = $this->db->prepare("SELECT o.id,o.numero,o.ano,o.valor_total,t.nome tipo
          FROM obrigacoes o
          JOIN tipos_obrigacao t ON t.id=o.tipo_obrigacao_id
          WHERE o.fornecedor_id=? AND o.ativo=1
          ORDER BY o.ano DESC,o.numero");
    ...
    public function parcelas(int $documentoId): array
    {
        $sql = "SELECT p.*,nd.codigo natureza_codigo,nd.nome natureza_nome,fr.codigo fonte_codigo,fr.nome fonte_nome,
            tr.codigo tipo_recurso_codigo,tr.nome tipo_recurso_nome,
    ...
    {
        $st = $this->db->prepare("SELECT fr.* FROM fontes_recurso fr
          JOIN obrigacao_fontes_recurso x ON x.fonte_recurso_id=fr.id
    ...
    {
        $st = $this->db->prepare("SELECT nd.* FROM naturezas_despesa nd
          JOIN obrigacao_naturezas_despesa x ON x.natureza_despesa_id=nd.id
    ...
    public function adicionarParcela(int $documentoId, array $d): int
    {
        ...
        if (!(bool)$doc['permite_avancar']) {
            throw new RuntimeException('A inspeção precisa estar em "Liberada liquidação de imposto" para programar parcelas.');
        }
        ...
        if ($empenho === '' || $natureza <= 0 || $exercicio < 2000 || $exercicio > 2100 || $fonte <= 0 || $tipoRecurso <= 0 || $valor === null || $valor <= 0 || !in_array($tipo, $tipos, true)) {
        ...
        $this->validarCadastroAtivo('tipos_recurso', $tipoRecurso, 'Tipo de recurso');
        ...
        try {
            ...
            $st->execute([
                ...
                mb_substr(trim((string)($d['historico_parcela'] ?? '')), 0, 255) ?: null,
                mb_substr(trim((string)($d['justificativa_ordem_cronologica'] ?? '')), 0, 150) ?: null,
                $this->usuarioId(),
            ]);
            $id = (int)$this->db->lastInsertId();
    ...
    {
        $sql = "SELECT p.*,d.obrigacao_id,d.numero documento_numero,d.valor_liquido valor_liquido_documento,
            td.nome tipo_documento,f.razao_social fornecedor,
            nd.codigo natureza_codigo,nd.nome natureza_nome,fr.codigo fonte_codigo,fr.nome fonte_nome,
            tr.codigo tipo_recurso_codigo,tr.nome tipo_recurso_nome,
            l.status status_liquidacao,l.data_liquidacao,c.status status_cmdf,c.data_conclusao data_cmdf,
            pg.status status_pagamento,pg.data_pagamento
          FROM parcelas_pagamento p
    ...
    public function atualizarLiquidacao(int $parcelaId, array $d): void
    {
        ...
        if (!in_array($status, $validos, true)) {
            throw new InvalidArgumentException('Status de liquidação inválido.');
        }
        ...
        if ($status === 'LIQUIDADA') {
            ...
            if (!$data) {
                throw new InvalidArgumentException('Informe a data de liquidação.');
            }
        } else {
            $data = null;
        }
        ...
        try {
            if ($status !== 'LIQUIDADA') {
                $down = $this->db->prepare("SELECT c.status cmdf_status,pg.status pagamento_status
                  FROM cmdf_etapas c LEFT JOIN pagamentos pg ON pg.parcela_id=c.parcela_id WHERE c.parcela_id=?");
                $down->execute([$parcelaId]);
                $downstream = $down->fetch();
            ...
            if ($q->rowCount() === 0) {
                ...
                if (!$exists->fetchColumn()) {
                    throw new RuntimeException('Registro de liquidação não encontrado.');
                }
            }

            if ($status === 'LIQUIDADA') {
                $this->db->prepare("INSERT INTO cmdf_etapas(parcela_id,status) VALUES(?,'AGUARDANDO')
                  ON DUPLICATE KEY UPDATE status=IF(status='LIQUIDADA','LIQUIDADA','AGUARDANDO'),data_conclusao=IF(status='LIQUIDADA',data_conclusao,NULL),atualizado_em=NOW()")
                  ->execute([$parcelaId]);
            }
            $this->db->commit();
        } catch (Throwable $e) {
    ...
    public function atualizarCmdf(int $parcelaId, array $d): void
    {
        ...
        $validos = ['AGUARDANDO','LIQUIDADA','CANCELADA','ANULADA'];
        if (!in_array($status, $validos, true)) {
            throw new InvalidArgumentException('Status de CMDF inválido.');
        }

        $st = $this->db->prepare("SELECT status FROM liquidacoes WHERE parcela_id=?");
        $st->execute([$parcelaId]);
        if ((string)$st->fetchColumn() !== 'LIQUIDADA') {
            throw new RuntimeException('Somente parcela liquidada pode seguir para CMDF.');
        }
        ...
        try {
            if ($status !== 'LIQUIDADA') {
                $pg = $this->db->prepare("SELECT status FROM pagamentos WHERE parcela_id=?");
                $pg->execute([$parcelaId]);
                if ($pg->fetchColumn() === 'PAGO') {
                    throw new RuntimeException('Não é possível alterar a CMDF porque a parcela já foi paga.');
                }
                $this->db->prepare("DELETE FROM pagamentos WHERE parcela_id=? AND status='AGUARDANDO'")->execute([$parcelaId]);
            }

            $exists = $this->db->prepare("SELECT id FROM cmdf_etapas WHERE parcela_id=?");
            $exists->execute([$parcelaId]);
            if ($exists->fetchColumn()) {
                $q = $this->db->prepare("UPDATE cmdf_etapas SET status=?,data_conclusao=?,usuario_id=?,atualizado_em=NOW() WHERE parcela_id=?");
                $q->execute([$status, $data, $this->usuarioId(), $parcelaId]);
            } else {
                $q = $this->db->prepare("INSERT INTO cmdf_etapas(parcela_id,status,data_conclusao,usuario_id) VALUES(?,?,?,?)");
                $q->execute([$parcelaId, $status, $data, $this->usuarioId()]);
            }

            if ($status === 'LIQUIDADA') {
                $this->db->prepare("INSERT INTO pagamentos(parcela_id,status) VALUES(?,'AGUARDANDO')
                  ON DUPLICATE KEY UPDATE status=IF(status='PAGO','PAGO','AGUARDANDO'),atualizado_em=NOW()")
                  ->execute([$parcelaId]);
            }
            $this->db->commit();
        } catch (Throwable $e) {
    ...
    private function validarVinculoObrigacao(string $tabela, string $coluna, int $obrigacaoId, int $id, string $rotulo): void
    {
        ...
        $st->execute([$obrigacaoId, $id]);
        if (!$st->fetchColumn()) {
            throw new RuntimeException($rotulo.' não está cadastrada nesta obrigação.');
        }
    }

    private function validarCadastroAtivo(string $tabela, int $id, string $rotulo): void
    {
        $permitidas = ['fornecedores','tipos_documento_pagamento','tipos_recurso'];
        if (!in_array($tabela, $permitidas, true)) {
            throw new LogicException('Tabela de validação não permitida.');
        }
        $st = $this->db->prepare("SELECT 1 FROM {$tabela} WHERE id=? AND ativo=1");
        $st->execute([$id]);
        if (!$st->fetchColumn()) {
            throw new InvalidArgumentException($rotulo.' inválido ou inativo.');
        }
    }
    ...
    private function validarCadastrosAtivos(string $tabela, array $ids, string $rotulo): void
    {
        ...
        if ((int)$st->fetchColumn() !== count($ids)) {
            throw new InvalidArgumentException($rotulo.' inválida ou inativa.');
        }
    }
