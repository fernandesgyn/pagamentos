...
final class Fila
{
    ...
    public function inspecoes():array
    {
        return $this->db->query("SELECT d.id documento_id,d.numero documento_numero,d.data_emissao,d.data_envio_cooinsp,d.valor_bruto,d.valor_liquido,d.data_lancamento,
            td.nome tipo_documento,f.razao_social fornecedor,o.numero obrigacao_numero,o.ano obrigacao_ano,
            s.nome status_inspecao,s.permite_avancar,i.data_conclusao
          FROM inspecoes i
    ...
    {
        return $this->db->query("SELECT d.id documento_id,d.numero documento_numero,d.valor_liquido,td.nome tipo_documento,
            f.razao_social fornecedor,s.nome status_inspecao,
    ...
    public function cmdf():array
    {
        return $this->db->query("SELECT d.id documento_id,d.numero documento_numero,td.nome tipo_documento,f.razao_social fornecedor,
          ...
          WHERE l.status='LIQUIDADA'
          ORDER BY CASE c.status WHEN 'AGUARDANDO' THEN 0 WHEN 'LIQUIDADA' THEN 1 ELSE 2 END,d.data_lancamento DESC,p.numero_parcela")->fetchAll();
    }
}
