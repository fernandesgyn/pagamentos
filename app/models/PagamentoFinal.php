...
final class PagamentoFinal
{
    ...
    public function listar():array
    {
        return $this->db->query("SELECT pg.*,p.numero_parcela,p.valor_liquido,p.numero_empenho,p.tipo,
            d.id documento_id,d.numero documento_numero,td.nome tipo_documento,f.razao_social fornecedor,
            c.data_conclusao data_cmdf
          FROM pagamentos pg
          ...
          JOIN cmdf_etapas c ON c.parcela_id=p.id AND c.status='LIQUIDADA'
    ...
    {
        $st=$this->db->prepare("SELECT c.status,p.valor_liquido,pg.id pagamento_id,pg.status pagamento_status
        ...
        $q=$this->db->prepare("UPDATE pagamentos SET status='PAGO',data_pagamento=?,valor_liquido_pago=?,historico_pagamento=?,benner_ap=?,usuario_id=?,atualizado_em=NOW() WHERE parcela_id=? AND status='AGUARDANDO'");
        $q->execute([
            $data,
            $valor,
