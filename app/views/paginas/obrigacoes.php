...
$tablePageSize=10;
$tableFilters=[
 ['label'=>'Pesquisa geral','column'=>'*','type'=>'search','placeholder'=>'Tipo, número, fornecedor ou SEI','class'=>'col-12 col-lg-4'],
...
<div class="card" data-admin-table data-page-size="<?=$tablePageSize?>">
  ...
  <div class="card-body p-0"><div class="table-responsive"><table class="table table-hover table-striped mb-0 align-middle">
    <thead><tr><th>Tipo</th><th>Nº/Ano</th><th>Fornecedor</th><th>Valor total</th><th class="text-end portal-actions-cell" data-table-nosort>Ações</th></tr></thead>
    <tbody>
      <?php foreach($obrigacoes as $o):?><tr data-record-id="<?=e($o['id'])?>">
        <td><?=e($o['tipo'])?></td>
        <td><strong><?=e($o['numero'])?>/<?=e($o['ano'])?></strong><?php if(!empty($o['nr_sei_contratacao'])):?><div class="small text-body-secondary">SEI <?=e($o['nr_sei_contratacao'])?></div><?php endif;?></td>
        <td><?=e($o['fornecedor'])?></td>
        <td class="money"><?=money($o['valor_total'])?></td>
        <td class="text-end portal-actions-cell"><div class="portal-action-group portal-table-actions">
          <a href="/documentos/novo?obrigacao_id=<?=e($o['id'])?>" class="btn btn-sm btn-outline-primary"><i class="fa-solid fa-receipt" aria-hidden="true"></i>Novo documento</a>
          <button type="button" class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#reversaoModal" data-reversao-modal data-reversao-action="/obrigacoes/<?=e($o['id'])?>/excluir" data-reversao-titulo="Excluir Obrigação" data-reversao-texto="A Obrigação só será excluída se não possuir Documentos vinculados. A exclusão ficará registrada na auditoria." data-reversao-botao="Excluir obrigação"><i class="fa-solid fa-trash me-1"></i>Excluir</button>
        </div></td>
      </tr><?php endforeach;?>
      <?php if(!$obrigacoes):?><tr data-table-empty><td colspan="5" class="text-center text-body-secondary py-4">Nenhuma obrigação cadastrada.</td></tr><?php endif;?>
    </tbody>
  </table></div></div>
