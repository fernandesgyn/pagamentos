...
$tablePageSize=10;
$tableFilters=[
 ['label'=>'Pesquisa geral','column'=>'*','type'=>'search','placeholder'=>'Tipo, número, fornecedor, SEI, fonte ou natureza','class'=>'col-12 col-lg-4'],
...
<div class="card" data-admin-table data-page-size="<?=$tablePageSize?>">
  ...
  <div class="card-body p-0"><div class="table-responsive"><table class="table table-hover table-striped mb-0 align-middle">
    <thead><tr>
      <th>Tipo</th><th>Nº/Ano</th><th>Fornecedor</th><th>Valor Total</th><th>Nr. SEI da Contratação</th><th>Fontes de recurso</th><th>Naturezas da despesa</th><th class="text-end portal-actions-cell" data-table-nosort>Ações</th>
    </tr></thead>
    <tbody>
      <?php foreach($obrigacoes as $o):?><tr data-record-id="<?=e($o['id'])?>">
        <td><?=e($o['tipo'])?></td>
        <td><strong><?=e($o['numero'])?>/<?=e($o['ano'])?></strong></td>
        <td><?=e($o['fornecedor'])?></td>
        <td class="money"><?=money($o['valor_total'])?></td>
        <td><?=e($o['nr_sei_contratacao']??'—')?></td>
        <td><?=e($o['fontes_recurso']??'—')?></td>
        <td><?=e($o['naturezas_despesa']??'—')?></td>
      ...
      <?php if(!$obrigacoes):?><tr data-table-empty><td colspan="8" class="text-center text-body-secondary py-4">Nenhuma obrigação cadastrada.</td></tr><?php endif;?>
    </tbody>
  </table></div></div>
