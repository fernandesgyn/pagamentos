...
$valorTotal=0.0;$temPago=false;foreach($parcelas as $p){$valorTotal+=(float)$p['valor_liquido'];if(($p['status_pagamento']??'')==='PAGO')$temPago=true;}
?>
...
<div class="card mb-3">
  ...
  <div class="card-body p-0"><div class="table-responsive"><table class="table table-hover mb-0 align-middle">
    <thead><tr><th>Documento / Parcela</th><th>Fornecedor</th><th>Atesto</th><th>Empenho</th><th>Valor</th><th class="text-end portal-actions-cell">Ações</th></tr></thead>
    <tbody>
      <?php foreach($parcelas as $p):?><tr>
        <td><strong><?=e($p['tipo_documento'])?> <?=e($p['documento_numero'])?></strong><div class="small">Parcela <?=e($p['numero_parcela'])?></div></td>
        <td><?=e($p['fornecedor'])?></td><td><?=e($p['data_atesto'])?></td><td><?=e($p['numero_empenho'])?></td><td><?=money($p['valor_liquido'])?></td>
        <td class="text-end portal-actions-cell"><div class="portal-action-group portal-table-actions">
          <a href="/documentos/<?=e($p['documento_id'])?>" class="btn btn-sm btn-outline-primary"><i class="fa-solid fa-file-lines me-1"></i>Documento</a>
          ...
          <?php if($status==='ATENDIDA'&&!empty($p['status_pagamento'])):?><a href="/pagamentos?cmdf_grupo_id=<?=e($grupo['id'])?>&parcela_numero=<?=e($p['numero_parcela'])?>" class="btn btn-sm btn-outline-success"><i class="fa-solid fa-money-check-dollar me-1"></i>Pagamento</a><?php endif;?>
        </div></td>
      </tr><?php endforeach;?>
      <?php if(!$parcelas):?><tr><td colspan="6" class="text-center text-body-secondary py-4">Grupo sem parcelas.</td></tr><?php endif;?>
    </tbody>
  </table></div></div>
</div>
...
<div class="card card-dark card-outline">
  ...
  <div class="card-body">
    <?php if($status==='FECHADA'):?><p class="mb-0">O grupo está <strong>Fechada</strong>. A composição ainda pode ser ajustada. Ao liberar, a composição fica bloqueada.</p>
    ...
    <?php else:?><div class="alert alert-success mb-0"><strong>CMDF Atendida.</strong> As parcelas foram liberadas individualmente para Pagamento.</div><?php endif;?>
  </div>
...
<?php if($status!=='FECHADA'):
  $reversaoAction='/cmdf/grupos/'.$grupo['id'].'/desfazer';
  $reversaoTitulo='Desfazer última ação da CMDF';
  $reversaoTexto=$status==='ATENDIDA'?'Volta o grupo de Atendida para Liberada e remove os pagamentos ainda não realizados.':'Volta o grupo de Liberada para Fechada, permitindo corrigir sua composição.';
  $reversaoBotao=$status==='ATENDIDA'?'Voltar para Liberada':'Voltar para Fechada';
  $reversaoBloqueio=($status==='ATENDIDA'&&$temPago)?'Há pagamento realizado em uma ou mais parcelas. Desfaça esses Pagamentos antes de voltar a CMDF.':'';
  require BASE_PATH.'/app/views/components/reversao_form.php';
endif;?>
<?php unset($status,$badge,$valorTotal,$temPago);?>
