...
<head>
  ...
  <style>
    .app-sidebar{--lte-sidebar-width:285px}.brand-text{font-size:1rem}.money{white-space:nowrap}
    .table td,.table th{vertical-align:middle}.card-title{font-weight:600}.summary-value{font-size:1.65rem;font-weight:700}
    .portal-page-actions{display:flex;align-items:center;justify-content:space-between;gap:.75rem;margin-bottom:1rem}
  </style>
</head>
...
<div class="app-wrapper">
  <nav class="app-header navbar navbar-expand bg-body border-bottom">
    <div class="container-fluid">
      <ul class="navbar-nav">
        ...
        <li class="nav-item d-none d-md-block"><a class="nav-link" href="/">Painel</a></li>
      </ul>
      <ul class="navbar-nav ms-auto align-items-center">
        <li class="nav-item d-none d-md-block"><span class="nav-link text-body-secondary"><?=e($user['nome']??'')?> · <?=e($user['perfil']??'')?></span></li>
        <li class="nav-item"><form method="post" action="/logout" class="m-0"><?=Csrf::field()?><button class="nav-link border-0 bg-transparent" type="submit"><i class="fa-solid fa-right-from-bracket me-1"></i>Sair</button></form></li>
      </ul>
    </div>
  </nav>

  <aside class="app-sidebar bg-body-secondary shadow" data-bs-theme="light">
    <div class="sidebar-brand"><a href="/" class="brand-link"><i class="brand-image fa-solid fa-file-invoice-dollar"></i><span class="brand-text fw-light">Liquidação e Pagamentos</span></a></div>
    <div class="sidebar-wrapper">
      <nav class="mt-2"><ul class="nav sidebar-menu flex-column" data-lte-toggle="treeview" data-accordion="false">
        ...
        <?php if(Auth::can('cmdf.gerir')):?><li class="nav-item"><a href="/cmdf" class="nav-link <?=$active('/cmdf')?>"><i class="nav-icon fa-solid fa-building-columns"></i><p>CMDF</p></a></li><?php endif;?>
        ...
        <?php if(Auth::can('cadastro.gerir')):?>
          ...
          <li class="nav-item"><a href="/tipos-recurso" class="nav-link <?=$active('/tipos-recurso')?>"><i class="nav-icon fa-solid fa-layer-group"></i><p>Tipos de recurso</p></a></li>
          ...
          <li class="nav-item"><a href="/tipos-obrigacao" class="nav-link <?=$active('/tipos-obrigacao')?>"><i class="nav-icon fa-solid fa-tags"></i><p>Tipos de obrigação</p></a></li>
        <?php endif;?>
  ...
  <main class="app-main">
    <div class="app-content-header"><div class="container-fluid"><h1 class="mb-0 fs-3"><?=e($titulo??'Painel')?></h1></div></div>
    <div class="app-content"><div class="container-fluid">
      <?php if($flash):?><div class="alert alert-<?=e($flash[0])?> alert-dismissible fade show"><?=e($flash[1])?><button class="btn-close" data-bs-dismiss="alert"></button></div><?php endif;?>
      <?=$conteudo?>
    </div></div>
  </main>
...
<script>document.querySelectorAll('form[method="post"],form[method="POST"]').forEach(function(f){if(f.querySelector('input[name="_token"]'))return;var i=document.createElement('input');i.type='hidden';i.name='_token';i.value=document.querySelector('meta[name="csrf-token"]').content;f.appendChild(i);});</script>
</body>
</html>
